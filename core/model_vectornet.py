...
class PointNetWithAgentHistory(rllib.template.Model):
    ...
    def forward(self, state: rllib.basic.Data, **kwargs):
        ...
        ego_embedding = self.ego_embedding(_state.ego, _state.ego_mask.to(torch.bool))
        ...
        obs_embedding = self.agent_embedding(_state.obs.flatten(end_dim=1), _state.obs_mask.to(torch.bool).flatten(end_dim=1))
...
def cut_state(state: rllib.basic.Data) :
    ...
    #change horizon stamp from 30 to custom horizon
    state_.ego[...,-1] = state_.ego[...,-1]*30/horizon
    state_.ego_mask = state_.ego_mask[:,-horizon:]

    state_.obs = state_.obs[:,:,-horizon:,:]
    #change horizon stamp from 30 to custom horizon
    state_.obs[...,-1] = state_.obs[...,-1]*30/horizon
