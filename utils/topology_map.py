...
from universe.common.topology_map import transform_points
from universe.common.global_path import calc_curvature_with_yaw_diff
class TopologyMapSampled(object):
    ...
    def build_graph(self):
        ...
        for i in range(len(self.centerline)):
            node_i = rllib.basic.Data(**self.graph.nodes[i])

            for j in range(len(self.centerline)):
                if i == j:
                    continue
                node_j = rllib.basic.Data(**self.graph.nodes[j])
    ...
    def render_graph(self, ax):
        pos = {key: rllib.basic.Data(**self.graph.nodes[key]).segment_center for key in self.graph.nodes}
